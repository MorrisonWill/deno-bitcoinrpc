...
export default class BitcoinRPC {
  ...
  async converttopsbt(
    ...
    return await this.request("converttopsbt", [
      hexstring,
      permitsigdata,
      iswitness,
    ]);
  ...
  async createpsbt(
    ...
    outputs: Array<
      { address: number } | { address: string } | { data: string }
    >,
    locktime?: number,
    replaceable?: boolean,
  ) {
    return await this.request("createpsbt", [
      inputs,
      outputs,
      locktime,
      replaceable,
  ...
  async createrawtransaction(
    inputs: Array<{ txid: string; vout: number; sequence?: number }>,
    output: Array<{ address: number } | { address: string } | { data: string }>,
    locktime?: number,
    replaceable?: boolean,
  ) {
